...
const checkBenefits = async (req, res) => {
	try {
		...
		if (!rows.length) {
			return res
				.status(200)
				.json({ status: "success", data: { is_partner: false } });
		}
...
const simulateTransaction = async (req, res) => {
	try {
		...
		const [[cuentaRows], [cashbackRows], [partnerRows]] = await Promise.all([
			...
			db.execute("SELECT monto_aprobado FROM cashback WHERE id_cliente = ?", [
				id_cliente,
			]),
			db.execute(
		...
		const cashback_rate = partnerRows[0]
			? Number(partnerRows[0].cashback_rate) / 100
			: 0;
		...
		const payment_plans = plazos.map((cuotas) => {
			...
			return {
				cuotas,
				monto_cuota: cuota,
				total: parseFloat((cuota * cuotas).toFixed(2)),
			};
		});
